...
```
Mac mini M4 (192.168.2.1)  ←→  Ubuntu PC (192.168.2.2)
  制御・Harvest・Git管理         GPU推論・パイプライン実行
  256GB SSD (容量少ない)         2TB SSD + 24GB VRAM
```

Ethernet直結で192.168.2.x/24のプライベートネットワーク。
...
ssh motomasahonda@192.168.2.2

# Ubuntu → Mac (専用鍵を生成)
ssh-keygen -t ed25519 -C "ubuntu-to-mac" -f ~/.ssh/id_ed25519_mac -N ""
ssh-copy-id -i ~/.ssh/id_ed25519_mac.pub motomasa@192.168.2.1
```
...
```json
// 旧形式 (2.2.0では動かない)
{"username":"hondamotomasa","key":"005eca56bb87758c47a3b5e5c5905cb8"}
```

新形式のAPIトークン（KaggleのSettings → API Tokens → 新しいトークンを生成）は文字列で発行される：

```
KGAT_e860b5561329b8f01de71448da1962b3
```
