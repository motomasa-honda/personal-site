...
- macOSのインターネット共有は `192.0.2.x` サブネットを自動割り当てする仕様
- 独自サブネット（198.51.100.x）を使っていたためNATが機能しなかった
- pfctlで手動NATを設定するも再起動で消える問題が発生
- launchdの永続化もタイミング問題で失敗し続けた
- 最終的にUbuntu側のIPを `192.0.2.2` に変更してmacOSの仕様に合わせて解決
...
もともとUbuntuのIPを `198.51.100.2`、MacのEthernetを `198.51.100.1` に設定していた。
...
# → nat on en1 inet from 192.0.2.0/24 to any -> (en1:0)
```

**macOSのインターネット共有は `192.0.2.0/24` に対してのみNATをかける仕様だった。**
`198.51.100.x` サブネットはNATの対象外なので当然通らない。

---

## pfctlで手動NATを追加してみた

`198.51.100.x` を維持したまま解決しようとpfに手動でNATルールを追加した。

```bash
# NATルールファイルを作成
sudo bash -c 'cat > /etc/pf.anchors/nat-linux << EOF
nat on en1 from 198.51.100.0/24 to any -> (en1)
pass all
EOF'
...
```bash
# Mac側のtcpdumpで確認
# en1に 192.0.2.8（MacのWi-Fi IP）→ 8.8.8.8 へのNATが確認できた
sudo tcpdump -i en1 -n icmp
# → 192.0.2.8 > 8.8.8.8: ICMP echo request ✅
# → 8.8.8.8 > 192.0.2.8: ICMP echo reply ✅
```
...
macOSのインターネット共有が `192.0.2.x` に対してNATをかける仕様だったので、
...
- **macOSのインターネット共有は `192.0.2.x` サブネット専用**。独自サブネットを使う場合はpfctlの手動設定が必要だが、再起動後の永続化が難しい
